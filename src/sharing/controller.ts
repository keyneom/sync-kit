 ...
 * read-modify-write against live local state, not a value assignment. Re-merge
 * inside the lock and return what was actually committed.
 *
 * The controller checks that the committed value subsumes the merged value
 * (merging `merged` into it adds nothing) and throws a `state` error
 * otherwise, so an apply that silently drops the merge fails loudly instead of
 * diverging from the cloud. Folding in edits newer than `read` is expected and
 * passes; the cloud picks them up on the next sync. Consumers with no local
 * mirror (a purely remote-derived dataset) may return `merged` unchanged.
 */
...
async function commitMerged<T>(
  ...
  // `apply` runs after the network write, so local state may have moved on
  // since `read`. Folding those newer edits in is correct and expected; what
  // is never acceptable is dropping part of the merge. Subsumption, not
  // equality: merging `merged` into what was committed must add nothing.
  if (
    codec.fingerprint(codec.merge(merged, committed)) !==
    codec.fingerprint(committed)
  ) {
    throw new SyncKitError(
      "state",
      `apply() for dataset ${datasetId} committed a value that drops part of ` +
        `the merge. Re-merge the merged value with live local state inside ` +
        `the lock that guards local edits, then return what was stored.`,
    );
  }
  return committed;
}
